  ...
  Future<void> fetchNotifications() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            'https://backendpint-5wnf.onrender.com/utilizadoresnotificacao/listarPorUser'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] != null && data['success']) {
          setState(() {
            ...
            selectedNotifications = List.filled(
                notifications.length, false); // Inicializa a lista de seleções
            notifications.sort((a, b) {
              DateTime dateA =
                  DateTime.parse(a['NOTIFICACAO']['DATA_HORA_NOTIFICACAO']);
              DateTime dateB =
                  DateTime.parse(b['NOTIFICACAO']['DATA_HORA_NOTIFICACAO']);
    ...
    bool? confirm = await showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text(
              'Tem a certeza de que deseja apagar as notificações selecionadas?'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
  ...
  Future<void> deleteNotification(int id) async {
    try {
      ...
      final response = await http.put(
        Uri.parse(
            'https://backendpint-5wnf.onrender.com/utilizadoresnotificacao/esconderTodasNotificacoesUtilizador'),
        headers: {'x-access-token': 'Bearer $token'},
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success']) {
          ...
          setState(() {
            notifications.removeWhere(
                (notification) => notification['ID_NOTIFICACAO'] == id);
          });
        ...
        print(
            'Falha ao apagar notificação. Status code: ${response.statusCode}');
      }
    } catch (e) {
      print(e);
    }
  ...
  void toggleSelection(int index) {
    setState(() {
      ...
      if (!selectedNotifications.contains(true)) {
        isSelectionMode =
            false; // Desativa o modo de seleção se nenhuma notificação estiver selecionada
    ...
    setState(() {
      if (allSelected) {
        selectedNotifications =
            List.filled(notifications.length, false); // Desmarca todas
        allSelected = false;
      } else {
        selectedNotifications =
            List.filled(notifications.length, true); // Marca todas
        allSelected = true;
      }
    });
  }
        ...
        child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  color: selectedNotifications[index]
                      ? Colors.grey[300]
                      : Colors.white,
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: NotificationCard(
                          title: notification['NOTIFICACAO']
                              ['TITULO_NOTIFICACAO'],
                          description: notification['NOTIFICACAO']
                                  ['MENSAGEM_NOTIFICACAO'] ??
                              'Sem descrição disponível',
                          date: notification['NOTIFICACAO']
                              ['DATA_HORA_NOTIFICACAO'],
                          eventId: notification['NOTIFICACAO']
                              ['ID_EVENTO'], // Passa o ID do evento
                          recommendationId: notification['NOTIFICACAO']
                              ['ID_RECOMENDACAO'], // Passa o ID da recomendação
                        ),
                      ),
                    ],
                  ),
